...
def backup():
    ...
    log.info("Preparing " + FILENAME + " from the directory...")

    # create compressed file from directory.
    with tempfile.NamedTemporaryFile() as t1:
        tar = tarfile.open(t1.name, "w|gz")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Backs up a directory or archives backups.')
    ...
    parser.add_argument('--DIR_PATH', required=True, help="Path to the directory to be backup up.")
    parser.add_argument('--ARCHIVE_NAME', default='directories', help='The base name for the archive')
